...
export default function ParentsManagement() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'completed': return 'bg-ivory-dark text-ink'
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-ink">Parents Management</h1>
          <p className="mt-1 text-sm text-ink-muted">Manage parent requests and assignments</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 sm:gap-4">
        {[
          { label: 'Total', count: parents.length, color: 'bg-ivory0' },
          { label: 'Pending', count: parents.filter(p => p.status === 'pending').length, color: 'bg-yellow-500' },
          { label: 'Qualified', count: parents.filter(p => p.status === 'qualified').length, color: 'bg-gold-500' },
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-ivory">
              <tr>
                ...
                <th className="hidden xl:table-cell px-6 py-3 text-left text-xs font-medium text-ink-muted uppercase tracking-wider">
                  Budget
                </th>
                <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-ink-muted uppercase tracking-wider">
                  Status
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredParents.map((parent) => {
                ...
                return (
                  <tr key={parent.id} className="hover:bg-ivory">
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-2 sm:ml-4">
                          <div className="text-sm font-medium text-ink">{parent.full_name}</div>
                          <div className="text-xs sm:text-sm text-ink-muted">Parent of {parent.child_name}</div>
                        </div>
                      </div>
                    </td>
                    <td className="hidden sm:table-cell px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-ink">{parent.email}</div>
                      <div className="text-sm text-ink-muted">{parent.phone}</div>
                    ...
                    <td className="hidden md:table-cell px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-ink">{parent.child_name}</div>
                      <div className="text-sm text-ink-muted">{parent.child_grade}</div>
                      <div className="text-sm text-ink-muted">Age: {parent.child_age}</div>
                    </td>
                    ...
                    <td className="hidden xl:table-cell px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-ink">{parent.budget_range}</div>
                      <div className="text-sm text-ink-muted">{parent.schedule_preferences}</div>
                    </td>
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-ink">Parent Details</h3>
              <button
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-ink">Parent Name</label>
                  <p className="text-sm text-ink">{selectedParent.full_name}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-ink">Child Name</label>
                  <p className="text-sm text-ink">{selectedParent.child_name}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-ink">{selectedParent.email}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-ink">Phone</label>
                  <p className="text-sm text-ink">{selectedParent.phone}</p>
                </div>
              </div>
              
              <div>
                <label className="block text-sm font-medium text-ink">Location</label>
                <p className="text-sm text-ink">{selectedParent.location}</p>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-ink">Child Grade</label>
                  <p className="text-sm text-ink">{selectedParent.child_grade}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-ink">Child Age</label>
                  <p className="text-sm text-ink">{selectedParent.child_age} years</p>
                </div>
              </div>
              
              <div>
                <label className="block text-sm font-medium text-ink">Requested Subjects</label>
              ...
              <div>
                ...
                <p className="text-sm text-ink">{selectedParent.budget_range}</p>
              </div>
              
              <div>
                <label className="block text-sm font-medium text-ink">Schedule Preferences</label>
                <p className="text-sm text-ink">{selectedParent.schedule_preferences}</p>
              </div>
              
              {selectedParent.special_requirements && (
                <div>
                  <label className="block text-sm font-medium text-ink">Special Requirements</label>
                  <p className="text-sm text-ink">{selectedParent.special_requirements}</p>
                </div>
              )}
            </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-lg w-full mx-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-ink">{selectedParent.full_name} - {selectedParent.child_name}</p>
              </div>
              
              <div>
                <label className="text-sm font-medium text-ink">Select Teacher</label>
                <select
                  value={selectedTeacher}
                  onChange={(e) => setSelectedTeacher(e.target.value)}
                  className="w-full mt-1 border border-ink/10 rounded-md px-3 py-2"
                >
              ...
              <div>
                <label className="text-sm font-medium text-ink">Assignment Notes</label>
                <textarea
              ...
              <div className="flex justify-end space-x-3">
                <button
                  onClick={() => setShowAssignModal(false)}
                  className="px-4 py-2 text-ink bg-gray-200 rounded-md hover:bg-gray-300"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-gold-500 text-white rounded-md hover:bg-gold-400 disabled:opacity-50"
                >
